...
func (s *Service) Update(id string, input *UpdateInput) (*core.Record, error) {
	...
	queryhelpers.SetOptional(record, "attackers_score_pct", input.AttackersScorePct, nil)
	queryhelpers.SetOptional(record, "defender_score_pct", input.DefenderScorePct, nil)
...
func (s *Service) applyCreateInput(record, system *core.Record, input *CreateInput, auth *core.Record) {
	...
	record.Set("webhook_id", strings.TrimSpace(input.WebhookID))
